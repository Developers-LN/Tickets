    ...
    public class TicketIdentifyModel
    {
        ...
        private object IdentifyNumberToObejct(IdentifyNumber n)
        {
            ...
            var identifyObject = new
            {
                ...
                && ((ra.Fraction >= n.FractionFrom && ra.Fraction <= n.FractionTo)
                    || ra.Award.ByFraction == (int)ByFractionEnum.N)
                    ).Select(s => s.Award.Value / (s.Raffle.Prospect.LeafFraction * s.Raffle.Prospect.LeafNumber)).FirstOrDefault(),
                ...
                && ((ra.Fraction >= n.FractionFrom && ra.Fraction <= n.FractionTo)
                    || ra.Award.ByFraction == (int)ByFractionEnum.N)
                    ).Select(s => (s.Award.Value / (s.Raffle.Prospect.LeafFraction * s.Raffle.Prospect.LeafNumber)) * ((n.FractionTo - n.FractionFrom) + 1)).FirstOrDefault(),
