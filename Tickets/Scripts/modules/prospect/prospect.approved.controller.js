/**=========================================================
 * Module: ProspectApproveeController.js
 =========================================================*/
...
(function() {
    ...
    angular
        .module('naut')
        .controller('ProspectApproveeController', ProspectApproveeController);

    ProspectApproveeController.$inject = ['$scope', '$rootScope', '$state', '$stateParams'];
    function ProspectApproveeController($scope, $rootScope, $state, $stateParams) {
        var self = this;
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.result === true) {
                        ...
                        $state.go('app.prospectWorkflowList');
                    } else {
        ...
        $scope.totalAwardPrice = 0;
        $scope.showTotal = function () {
            var total = 0;
            $scope.workflow.prospect.awards.forEach(function (award) {
                total += award.totalValue;
            });
            $scope.totalAwardPrice = total;
        }
        ...
        this.loadWorkflow = function () {
            ...
            $.ajax({
                ...
                url: $rootScope.serverUrl + 'ticket/workflowApi/getWorkflow?id=' + $stateParams.id,
                success: function (response) {
                    window.loading.hide();
                    if (response.result == true) {
